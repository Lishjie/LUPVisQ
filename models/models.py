...
class Objective(nn.Module):
    """
    Objective network for learning Objective aesthetic features

    Args:
        lda_out_channels: local distortion aware module output size.
        target_in_size: input vector size for target network (Fully connected network).
        target_fc(i)_size: fully connection layer size of target network (Fully connected network).
    ...
    def __init__(self, lda_out_channels, target_in_size, target_fc1_size, target_fc2_size, target_fc3_size, target_fc4_size):
        ...
        # ResNet50 backbone
        self.res = resnet50_backbone_(lda_out_channels, target_in_size, pretrained=True)

        self.pool = nn.AdaptiveAvgPool2d((1, 1))

        # Fully connected layers
        self.l1 = nn.Sequential(
            nn.Linear(self.target_in_size, self.f1),
            nn.ReLU(True),
        )
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.relu = nn.ReLU(inplace=True)  # inplace的作用是节省内存
...
class ResNetBackbone(nn.Module):
    ...
    in_chn: 224
    block: Bottleneck
    layers: [3, 4, 6, 3]  每一个stage重复的次数，具体参考ResNet50的网络结构设置
    num_classes: 1000
    """
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        downsample = None
        # 如果当前layer存在channels数量的变化，则设置残差模块downsample
    ...
    def forward(self, x):
        ...
        vec = torch.cat((lda_1, lda_2, lda_3, lda_4), 1)  # 向量拼接函数

        out = {}
        out['hyper_in_feat'] = x  # 超网的输入数据
        out['target_in_vec'] = vec  # target网络的输入数据

        return out


class ResNetBackbone_(nn.Module):
    """
    Original ResNet50 Network
    """
    def __init__(self, lda_out_channels, in_chn, block, layers, num_classes=1000):
        super(ResNetBackbone_, self).__init__()
        # conv_1
        self.inplanes = 64
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
...
def resnet50_backbone_(lda_out_channels, in_chn, pretrained=False, **kwargs):
    """
    Constructs a ResNet-50 model

    Args:
        pertrained (bool): If Ture, returns a model pre-trained on ImageNet
    """
    model = ResNetBackbone_(lda_out_channels, in_chn, Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
        save_model = model_zoo.load_url(model_urls['resnet50'])
        model_dict = model.state_dict()
        state_dict = {k: v for k, v in save_model.items() if k in model_dict.keys()}
