...
class ObjectiveNet(nn.Module):
    ...
    Modules.ObjectiveNet(16, 224, 112, 56, 28, 14)
    """
    def __init__(self, lda_out_channels, linear_in_size, linear_fc1_size, linear_fc2_size, linear_fc3_size, linear_fc4_size):
        super(ObjectiveNet, self).__init__()

        self.linear_in_size = linear_in_size
        self.f1 = linear_fc1_size
        self.f2 = linear_fc2_size
        self.f3 = linear_fc3_size
        self.f4 = linear_fc4_size
        ...
        self.l1 = nn.Sequential(
            nn.Linear(self.linear_in_size, self.f1),
            nn.ReLU(True),
        )
        self.l2 = nn.Sequential(  # different from the pre-train network parameters
...
class Bottleneck(nn.Module):
    expansion = 4 # 扩张倍数，对应每一layer的最终输出channels数
...
class ResNetBackbone(nn.Module):
    ...
    def __init__(self, lda_out_channels, in_chn, block, layers, num_classes=1000):
        ...
        self.bn1 = nn.BatchNorm2d(64)  # 对经过卷积的数据进行标准化
        self.relu = nn.ReLU(inplace=True)  # 卷积层经过ReLu激活函数
...
def subjectiveNet_backbone(lda_out_channels=16, hyper_in_channels=112, target_in_size=224, target_fc1_size=112, target_fc2_size=56, target_fc3_size=28, target_fc4_size=14, feature_size=7, pretrain=False):
    ...
    model = SubjectiveNet(lda_out_channels, hyper_in_channels, target_in_size, target_fc1_size, target_fc2_size, target_fc3_size, target_fc4_size, feature_size)
    if pretrain:
